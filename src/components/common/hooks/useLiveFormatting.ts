...
const useLiveFormatting = ({
  ...
  // const escapeMarker = useCallback((event: KeyboardEvent) => {
  //   const el = inputRef.current;
  //   if (!el) return;
  //   const sel = window.getSelection();
  //   if (!sel?.isCollapsed || !sel.anchorNode || !el.contains(sel.anchorNode)) return;
  ...
  //     const markerElement = sel.anchorNode.parentElement;
  //     const anchorTextLength = sel.anchorNode.textContent?.length || 0;
  //     const caretPosition = sel.anchorOffset;
  //     // Determine if we should move left or right based on caret position
  //     const moveLeft = caretPosition <= anchorTextLength / 2;

  //     // Get the previous or next sibling text node
  //     // eslint-disable-next-line no-null/no-null
  //     let targetNode: Node | null = null;
  //     let targetOffset = 0;

  //     if (moveLeft) {
  //       // Find the closest previous text node
  //       let current = markerElement;
  //       while (current.previousSibling) {
  //         if (current.previousSibling.nodeType === Node.TEXT_NODE) {
  //           targetNode = current.previousSibling;
  //           targetOffset = (current.previousSibling.textContent || '').length;
  //           break;
  //         }
  ...
  //         markerElement.parentNode?.insertBefore(targetNode, markerElement);
  //         targetOffset = 0;
  //       }

  //       // Prepend the key to the text node
  //       targetNode.textContent = (targetNode.textContent || '') + event.key;
  //       targetOffset = targetNode.textContent.length;
  //     } else {
  //       // Find the closest next text node
  //       let current = markerElement;
  //       while (current.nextSibling) {
  //         if (current.nextSibling.nodeType === Node.TEXT_NODE) {
  //           targetNode = current.nextSibling;
  //           targetOffset = 0;
  //           break;
  //         }
  ...
  //           markerElement.parentNode?.insertBefore(targetNode, markerElement.nextSibling);
  //         } else {
  //           markerElement.parentNode?.appendChild(targetNode);
  //         }
  //         targetOffset = 0;
  //       }

  //       // Append the key to the text node
  //       targetNode.textContent = event.key + (targetNode.textContent || '');
  //       targetOffset += event.key.length;
  //     }

  //     // Move the caret to the target node
  //     if (targetNode) {
  //       const range = document.createRange();
