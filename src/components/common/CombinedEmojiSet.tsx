...
import type { CombinedEmojiSetData } from './CombinedEmojiPicker';
...
const CombinedEmojiSet: FC<OwnProps & StateProps> = ({
  set,
  loadAndPlay,
  index,
  idPrefix,
  ...
  const isRecent = set.id === RECENT_SYMBOL_SET_ID;
  const isStatusCollectible = set.id === COLLECTIBLE_STATUS_SET_ID;
  const isFavorite = set.id === FAVORITE_SYMBOL_SET_ID;
  const isPopular = set.id === POPULAR_SYMBOL_SET_ID;
  const isEmoji = set.isEmoji;
  const isPremiumSet = !isRecent && selectIsSetPremium(set);
  ...
  useEffect(() => {
    if (shouldRender && !set.stickers?.length && set.accessHash) {
      loadStickers({
        stickerSetInfo: {
          id: set.id,
  ...
  }, [shouldRender, loadStickers, set]);

  const isLocked = !isSavedMessages && !isCurrentUserPremium && isPremiumSet && !isChatEmojiSet;

  // const isInstalled = set.installedDate && !set.isArchived;
  const isInstalled = true; // assume true?

  const canCut = !isInstalled && set.id !== RECENT_SYMBOL_SET_ID
    && set.id !== POPULAR_SYMBOL_SET_ID && set.id !== EFFECT_EMOJIS_SET_ID
    && set.id !== EFFECT_STICKERS_SET_ID && !isChatEmojiSet && !isChatStickerSet;

  const [isCut, , expand] = useFlag(canCut);
  const itemsBeforeCutout = itemsPerRow * 3 - 1;
  const totalItemsCount = (withDefaultTopicIcon || withDefaultStatusIcon) ? set.count + 1 : set.count;
    ...
    >
      ...
      <div
        className={buildClassName(
          ...
          set.id === EFFECT_EMOJIS_SET_ID && 'effect-emojis',
        )}
      ...
      >
        ...
        {shouldRender && set.reactions?.map((reaction) => {
          const reactionId = getReactionKey(reaction);
          const isSelected = reactionId ? selectedReactionIds?.includes(reactionId) : undefined;

          return (
            <ReactionEmoji
              key={`${set.id}_${reactionId}`}
              reaction={reaction}
          ...
          .map((sticker, i) => {
            ...
            return (
              <StickerButton
                ...
                forcePlayback={forcePlayback}
                isEffectEmoji={set.id === EFFECT_EMOJIS_SET_ID}
                noShowPremium={isCurrentUserPremium
                  && (set.id === EFFECT_STICKERS_SET_ID || set.id === EFFECT_EMOJIS_SET_ID)}
